...
const ChatQuickReport = () => {
  ...
  return <div className="w-full max-w-4xl mx-auto mb-8 px-4">
      <Card className={`bg-black/80 border-2 transition-all duration-300 backdrop-blur-sm ${isFocused ? 'border-red-500/70 shadow-lg shadow-red-500/20' : 'border-white/20'}`}>
        <div className="p-4 sm:p-6 bg-[#1d3845]">
          <div className="flex items-center space-x-3 mb-4">
            ...
            {qualityScore === 'poor' && characterCount > 0 && <div className="flex items-center space-x-1 text-yellow-400 text-xs">
                <AlertTriangle className="w-3 h-3" />
                <span>Add more detail</span>
              </div>}
            {hasTechnicalContent && <div className="flex items-center space-x-1 text-green-400 text-xs">
          ...
          <form onSubmit={handleSubmit} className="space-y-4">
            <div className="flex flex-col sm:flex-row sm:items-center space-y-2 sm:space-y-0 sm:space-x-3">
              ...
              <Select value={period} onValueChange={setPeriod}>
                ...
                <SelectContent className="bg-black/90 border-white/20">
                  {periodOptions.map(option => <SelectItem key={option.value} value={option.value} className="text-white text-xs sm:text-sm">
                      {option.label}
                    </SelectItem>)}
                </SelectContent>
              </Select>
            ...
            <div className="flex flex-col sm:flex-row space-y-3 sm:space-y-0 sm:space-x-3">
              <div className="flex-1">
                <Textarea value={report} onChange={e => setReport(e.target.value)} onFocus={() => setIsFocused(true)} onBlur={() => setIsFocused(false)} placeholder="For better AI scoring include: technical implementation details, specific completed features, challenges solved, technology stack, measurable progress, and next steps..." className={`bg-black/20 border-white/20 text-white placeholder:text-gray-500 font-mono resize-none transition-all duration-300 text-xs sm:text-sm ${isFocused ? 'border-red-500/70' : ''}`} rows={4} disabled={isSubmitting} />
              ...
              <Button type="submit" disabled={isSubmitting || !report.trim() || !period} className={`px-4 sm:px-6 py-2 sm:py-3 font-mono self-end w-full sm:w-auto text-xs sm:text-sm ${isSubmitting ? 'opacity-50 cursor-not-allowed' : ''} ${report.trim() && period ? 'bg-red-600 hover:bg-red-700' : 'bg-gray-600 hover:bg-gray-700'}`}>
                {isSubmitting ? <>
