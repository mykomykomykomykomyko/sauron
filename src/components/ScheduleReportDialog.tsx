      ...
      <DialogContent className="max-w-4xl max-h-[90vh] overflow-y-auto bg-black/95 border-white/20 text-white">
        ...
        <div className="space-y-6 py-4">
          ...
          <Card className="bg-black/40 border-white/20">
            <CardHeader>
              <CardTitle className="text-lg font-mono flex items-center space-x-2">
                ...
                <span className="text-slate-50">Select Contractors</span>
                {selectedContractors.length > 0 && <Badge className="bg-green-500/20 text-green-400">
            ...
            <CardContent>
              {isLoading ? <div className="space-y-2">
                  {[...Array(3)].map((_, i) => <div key={i} className="animate-pulse h-12 bg-white/10 rounded"></div>)}
                </div> : <div className="grid grid-cols-1 md:grid-cols-2 gap-3 max-h-48 overflow-y-auto">
                  {contractors.map(contractor => <div key={contractor.id} onClick={() => toggleContractor(contractor)} className={`p-3 border rounded-lg cursor-pointer transition-all ${contractor.selected ? 'border-green-500/50 bg-green-500/20' : 'border-white/20 hover:border-white/40 bg-black/20'}`}>
                      <div className="flex items-center justify-between">
                        <div>
                          ...
                          {contractor.company_name && <div className="text-xs text-gray-500">{contractor.company_name}</div>}
                        </div>
                        {contractor.selected && <div className="w-4 h-4 bg-green-500 rounded-full flex items-center justify-center">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            <Card className="bg-black/40 border-white/20">
              <CardHeader>
                <CardTitle className="text-lg font-mono flex items-center space-x-2 text-slate-50">
                  <Clock className="w-5 h-5 text-yellow-400" />
                  <span>Due Date</span>
                </CardTitle>
              </CardHeader>
              <CardContent>
                <Input type="date" value={dueDate} onChange={e => setDueDate(e.target.value)} className="bg-neutral-900/50 border-white/20 text-white font-mono" />
              </CardContent>
            </Card>
            ...
            <Card className="bg-black/40 border-white/20">
              <CardHeader>
                <CardTitle className="text-lg font-mono flex items-center space-x-2">
                  ...
                  <span className="text-slate-50">Report Period</span>
                </CardTitle>
              </CardHeader>
              <CardContent>
          ...
          <Card className="bg-black/40 border-white/20">
            <CardHeader>
              <CardTitle className="text-lg font-mono flex items-center space-x-2">
                ...
                <span className="text-slate-50">Email Content</span>
              </CardTitle>
            </CardHeader>
            <CardContent className="space-y-4">
              <div>
                <label className="text-sm text-gray-300 font-mono">Custom Subject (optional)</label>
                <Input value={customSubject} onChange={e => setCustomSubject(e.target.value)} placeholder={getEmailSubject()} className="bg-neutral-900/50 border-white/20 text-white font-mono mt-1" />
              </div>

              <div>
                <div className="flex items-center justify-between mb-2">
                  <label className="text-sm text-gray-300 font-mono">AI Context (for generation)</label>
                  <Button onClick={generateAIMessage} disabled={isGenerating || !aiPrompt.trim()} size="sm" className="bg-purple-600 hover:bg-purple-700 text-white font-mono">
                    {isGenerating ? <>
                ...
                <Textarea value={aiPrompt} onChange={e => setAiPrompt(e.target.value)} placeholder="Describe the projects, specific areas to focus on, or any context for the AI to generate a personalized message..." className="bg-neutral-900/50 border-white/20 text-white font-mono resize-none h-20" />
              </div>

              <div>
                <label className="text-sm text-gray-300 font-mono">Custom Message (optional)</label>
                <Textarea value={customMessage} onChange={e => setCustomMessage(e.target.value)} placeholder="Leave empty to use default message template..." className="bg-neutral-900/50 border-white/20 text-white font-mono resize-none h-32 mt-1" />
              </div>
            </CardContent>
          </Card>

          {/* Preview */}
          {showPreview && <Card className="bg-black/40 border-white/20">
              <CardHeader>
                <CardTitle className="text-lg font-mono flex items-center space-x-2">
          ...
          <div className="flex items-center justify-between pt-4 border-t border-white/20">
            <Button onClick={() => setShowPreview(!showPreview)} variant="outline" className="border-white/20 text-white/90 hover:bg-white/10 font-mono">
              <Eye className="w-4 h-4 mr-2" />
              {showPreview ? 'Hide Preview' : 'Show Preview'}
            </Button>

            <div className="flex space-x-3">
              <Button onClick={() => onOpenChange(false)} variant="outline" className="border-white/20 text-white/90 hover:bg-white/10 font-mono">
                Cancel
              </Button>
              <Button onClick={sendEmails} disabled={selectedContractors.length === 0} className="bg-gradient-to-r from-red-600 to-purple-600 hover:from-red-700 hover:to-purple-700 text-white font-mono">
