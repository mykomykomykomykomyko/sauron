...
serve(async (req) => {
  ...
  try {
    ...
    // Enhanced analysis prompt with scoring aligned to 0-1000 range
...
Be extremely strict. No work done = low score. Meetings without outputs = flags. Demand concrete technical deliverables.`;
    ...
    try {
      ...
      // Ensure score is within 0-1000 range (updated range)
      ...
      // Fallback analysis with strict evaluation (updated to 1000 scale)
      ...
      let fallbackScore = 200; // Start low (out of 1000)
      if (hasDeliverables) fallbackScore += 200;
      if (reportLength > 100) fallbackScore += 150;
      
      analysis = {
        score: Math.min(1000, fallbackScore), // Ensure max 1000
    ...
    const { data: analysisResult, error: analysisError } = await supabase
      ...
      .upsert({
        report_id: reportId,
        score: analysis.score, // Now properly in 0-1000 range
